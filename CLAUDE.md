...
- **Entry points.** Built. `main.py` parses arguments, delegates, and prints a summary; it exits 0 on a clean batch, 1 when any document failed or none were found, and 2 on a configuration problem. `app.py` runs the same orchestrator, caching the workflow with `st.cache_resource` and staging uploads to a temp folder so the loaders read them by path exactly as the CLI does. `api/index.py` is a third interface, a FastAPI app deployed to Vercel; it is one-document-per-request because a batch does not fit a function timeout, writes only to `/tmp` because the deployed filesystem is read-only, and never imports `reporting` — that is what keeps pandas and streamlit out of the serverless bundle, which the 250 MB limit requires. Its dependency set lives in `api/requirements.txt`, deliberately not a copy of the root one; keep the shared pins in step.
