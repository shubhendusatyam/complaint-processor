...
- **Entry points.** Built. `main.py` parses arguments, delegates, and prints a summary; it exits 0 on a clean batch, 1 when any document failed or none were found, and 2 on a configuration problem. `app.py` runs the same orchestrator, caching the workflow with `st.cache_resource` and staging uploads to a temp folder so the loaders read them by path exactly as the CLI does. `api/index.py` is a third interface, a FastAPI app deployed to Vercel; it is one-document-per-request because a batch does not fit a function timeout, and writes only to `/tmp` because the deployed filesystem is read-only. **Its dependencies belong in the root `requirements.txt`** — the Python runtime reads only the project root, so a `requirements.txt` beside the entrypoint is silently ignored and the function dies on `ModuleNotFoundError`. Nothing needs listing to be bundled: every reachable project file ships, and `excludeFiles` in `vercel.json` trims what should not.
